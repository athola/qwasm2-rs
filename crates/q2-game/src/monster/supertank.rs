//! Supertank monster. C ref: ~/Qwasm2/src/game/monster/supertank/
use q2_shared::types::*;
use crate::ai;
use crate::constants::*;
...
pub fn sp_monster_supertank(storage: &mut EntityStorage, key: EntityKey, fields: &HashMap<String, String>) {
    if let Some(ent) = storage.get_mut(key) {
        ent.game.classname = fields.get("classname").cloned().unwrap_or_else(|| "monster_supertank".to_string());
