//! Floater monster. C ref: ~/Qwasm2/src/game/monster/float/
use q2_shared::types::*;
use crate::ai;
use crate::constants::*;
...
pub fn sp_monster_floater(storage: &mut EntityStorage, key: EntityKey, fields: &HashMap<String, String>) {
    if let Some(ent) = storage.get_mut(key) {
        ent.game.classname = fields.get("classname").cloned().unwrap_or_else(|| "monster_floater".to_string());
