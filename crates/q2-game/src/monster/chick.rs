//! Chick (Iron Maiden) monster. C ref: ~/Qwasm2/src/game/monster/chick/
use q2_shared::types::*;
use crate::ai;
use crate::constants::*;
...
pub fn sp_monster_chick(storage: &mut EntityStorage, key: EntityKey, fields: &HashMap<String, String>) {
    if let Some(ent) = storage.get_mut(key) {
        ent.game.classname = fields.get("classname").cloned().unwrap_or_else(|| "monster_chick".to_string());
